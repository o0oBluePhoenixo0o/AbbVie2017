...
with open('twitter_preprocessing_0720.csv', 'w', encoding='UTF-16LE', newline='') as csvfile:
    column = [['key', 'created_time', 'language','message','re_message']]
...
df_rm = df_postncomment.drop_duplicates(subset=['key','re_message'])
rm_english_na = df_rm.dropna()
#display(rm_english_na)
rm_english_na.index=range(len(rm_english_na))
dfinal_tw = pd.DataFrame(rm_english_na, columns = ['key', 'created_time', 'language','message','re_message'])
dfinal_tw.to_csv("final_twitter_preprocessing_0720.csv", encoding='UTF-16LE',columns = ['key', 'created_time', 'language','message','re_message'])
...
pyLDAvis.save_html(vis_data, 'lda_tw40_0720.html')
...
#import os
#os.remove('final_twitter_preprocessing_0720.csv')
#os.remove('twitter_preprocessing_0720.csv')