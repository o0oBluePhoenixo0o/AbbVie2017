...
#tmp <- read.csv("Final_Manual_3007.csv",as.is = TRUE, sep = ",")

# correct encoding#
tmp <- subset(tmp,select = c("message","Id"))
tmp <- tmp %>%
  mutate(message = iconv(message, "latin1", "ASCII", "byte"), stringAsFactors = FALSE)

tmp <- unique(tmp)

## ---- most used emoji ----
# rank emojis by occurence in data
rank <- emojis_matching(tmp$message, matchto, description) %>% 
