...
myStopwords <- c(stopwords("SMART"),"rt","via","amp","aa","ra","und","im",
                 "abbv","amgn","boris","today","abbvies","llc","inc",
                 "abbvie","amgen","adalimumab","ankylosing","spondylitis",
                 "bristol","myers","enbrel","hepatitis","humira","trilipix","bristolmyers","bmy",
                 "ibrutinib","imbruvica","johnson","psoriasis","rheumatoid","arthritis"
                 )
...
bestK <- function(b_dtm = dtm, b_k = k_num, b_seed = SEED)
  ...
  for(k in b_k)
  {
    ...
    #                                                            thin = 100,    iter = 1000)) 
    perplexity_CTM <- c(perplexity_CTM, perplexity(ap_CTM))
  }
...
# Pick a random seed for replication
SEED = sample(1:length(preprocess_final),10000)


# Find the k that can achieve the smallest perplexity
k = k_num[which.min(k_perplexity)]
