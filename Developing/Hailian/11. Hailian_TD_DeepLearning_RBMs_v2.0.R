...
########################################################################################3
#Preprocessing and construct DTM
...
clean <- function (sentence){
  ...
  removeURL <- function(x) gsub('"(http.*) |(https.*) |(http.*)$|\n|can|just|do|now|will|get|one|dont|also|much|really|even|use|got|still|year|cant|new|going|since|every|want|by|about|as|so|it|may|â|ã|ãƒâ|šã|å|ãƒæ|iãƒæ|â€š|rt|ed|fc|bd|bc', "", x)
...
Final_TW_Tweets$Text <- gsub(",", "", Final_TW_Tweets$Text)  # 
...
t1<- Sys.time()
dtm_test1<- reduceDims(dtm_dataset_binaryweight, 500, 100)
# if there are more than 10000 terms, select terms with frequency > 500
...
Topic_20<- rbm(num_hidden=20, training_data=weightMatrix, learning_rate=.09, n_iterations=100,
                               mini_batch_size=sample(divisors(dim(weightMatrix)[2]),1), momentum=0.9)
Topic_20_T<- t(Topic_20)
