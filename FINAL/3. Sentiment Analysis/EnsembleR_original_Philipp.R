...
df <- read.csv(file.choose(), encoding = "UTF-8", header = TRUE, sep = ",", stringsAsFactors = F)
df <- unique(df)
...
#BingLiu
pos.words <- scan('./input_data/Positive.txt', what='character', comment.char=';') #folder with positive dictionary
neg.words <- scan('./input_data/Negative.txt', what='character', comment.char=';') #folder with negative dictionary
