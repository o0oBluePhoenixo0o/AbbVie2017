## Developing Sarcasm Detection models using different algorithms (DRF, GBM, DL, NB)
## Using manual label dataset and the sarcasm label dataset obtained from Edwison.com (TweetDataSet.csv)
...
manual_test <- read.csv(file.choose(), encoding = "UTF-8", header = TRUE, sep = ",", stringsAsFactors = F)
...
# Use the file "5.Dataset/TweetsDataSet.csv"
TW_df <- read.csv(file.choose(),stringsAsFactors = F)
...
rf.model     <- h2o.randomForest(x=x, y=y, training_frame = h.train, ntrees = 1000, nfolds = 10)
...
deep_learning <- h2o.deeplearning( x = x,
                                   ...
                                   nfolds = 10,
                                   adaptive_rate =F)
pred_dl <- as.data.frame(h2o.predict(deep_learning,h.test))
metrics(table('Actual class' = test[,1], 'Predicted class' = pred_dl$predict ))
...
metrics(table('Actual class' = test[,1], 'Predicted class' = n.pred ))
...
metrics(table('Actual class' = test[,1], 'Predicted class' = n.pred.lap ))
########################################################################
